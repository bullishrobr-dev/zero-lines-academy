...
const syringePricingQuiz: GeneralQuiz = {
  ...
  questions: [
    ...
    {
      question: 'You have named the {locationName} price of {currency}300. What is Offer Option 1?',
      questionEs: 'Ya has dicho el precio de {locationName}, {currency}300. ¿Cuál es la Opción de Oferta 1?',
      options: [
    ...
    },
    {
      ...
      correctIndex: 2,
      explanation: 'The absolute minimum is {currency}100 — and it is not yours to give. The descent runs {currency}175 (gift removed) → {currency}140 (the 20% voucher, used ONCE, there is no second one) → and below that you go full market and CALL A MANAGER. He says {currency}100, not you. A seller who can reach the floor on their own has no floor.',
      explanationEs: 'El mínimo absoluto es {currency}100 — y no es tuyo para darlo. La bajada va {currency}175 (sin regalo) → {currency}140 (el cupón del 20%, UNA vez, no hay un segundo) → y por debajo vas a mercado puro y LLAMAS AL ENCARGADO. Los {currency}100 los dice él, no tú. Un vendedor que llega solo al fondo no tiene fondo.',
    },
  ],
};
...
const stoppingQuiz: GeneralQuiz = {
  ...
  description: 'Master the art of stopping customers — timing, approaches, product hooks, and getting the look before you move.',
  descriptionEs: 'Domina el arte de parar a los clientes — momento, enfoques, ganchos específicos por producto y conseguir la mirada antes de moverte.',
  icon: 'Hand',
  category: 'Stopping',
  categoryEs: 'Parada',
  xpReward: 60,
  questions: [
    ...
    {
      question: 'You call "Hi guys, how you doing?" to a woman four or five metres off and she does not look up. What do you do?',
      questionEs: 'Le sueltas "Hola chicos, ¿qué tal?" a una mujer que está a cuatro o cinco metros y no levanta la vista. ¿Qué haces?',
      options: [
        'Start walking at her anyway with the sample already out in front of you',
        'Call again, louder, until she has to acknowledge that you are talking to her',
        'Step into the middle of the walkway so that she has to deal with you somehow',
        'Nothing — you committed nothing, so greet the next one coming',
      ],
      optionsEs: [
        'Andar hacia ella igualmente, con la muestra ya por delante',
        'Llamarla otra vez, más alto, hasta que no le quede más remedio que mirarte',
        'Ponerte en medio del pasillo para que tenga que lidiar contigo sí o sí',
        'Nada — no te has comprometido a nada, saluda al siguiente que venga',
      ],
      correctIndex: 3,
      explanation: 'You want their eyes on you BEFORE you take a step. No look, no step. You committed nothing, so nothing was lost — and walking at someone who has not looked at you is how you turn into the person everyone crosses the floor to avoid. Greet the next one, same voice, same smile.',
      explanationEs: 'Quieres que te miren ANTES de dar un paso. Sin mirada, no te mueves. No te has comprometido a nada, así que no has perdido nada — y andar hacia alguien que no te ha mirado es cómo te conviertes en la persona que todo el mundo esquiva. Saluda al siguiente, misma voz, misma sonrisa.',
    },
    {
...
const closingQuiz: GeneralQuiz = {
  ...
  questions: [
    ...
    {
      question: 'You have named {currency}300 for the syringe and she has gone quiet without asking you for anything. What comes next?',
      questionEs: 'Has dicho {currency}300 por la jeringuilla y se ha quedado callada sin pedirte nada. ¿Qué viene ahora?',
      options: [
        'Immediately drop to {currency}100, the absolute floor, before she has had time to hand the syringe back to you',
      ...
      optionsEs: [
        'Bajar de inmediato a {currency}100, que es el mínimo absoluto, antes de que le dé tiempo siquiera a devolverte la jeringuilla a la mano',
      ...
      correctIndex: 1,
      explanation: 'The two offers are the next beat of the pitch, not a reward for going quiet — every customer hears them, which is exactly why you never have to invent a discount. What a silence does NOT buy is the rungs underneath: {currency}175, {currency}140 and {currency}100 are answers to an ASK, and you make her ask twice before you move once. Drop on a pause and you have taught her that the numbers fall whenever she stops talking, and she will stop talking all the way to the floor.',
      explanationEs: 'Las dos ofertas son el siguiente paso del pitch, no un premio por quedarse callada — las oye todo el mundo, y por eso justamente nunca tienes que inventarte un descuento. Lo que un silencio NO compra son los escalones de abajo: {currency}175, {currency}140 y {currency}100 responden a una PETICIÓN, y que te la pida dos veces antes de moverte una. Si bajas por una pausa le has enseñado que los números caen cada vez que se calla, y se va a callar hasta el suelo.',
    },
  ],
};
...
const objectionQuiz: GeneralQuiz = {
  ...
  questions: [
    {
      question: 'You are still up at {currency}210 with the gift on and she says: "It is too expensive." What is your FIRST move?',
      questionEs: 'Sigues arriba, en {currency}210 con el regalo puesto, y te dice: "Es demasiado caro." ¿Cuál es tu PRIMER movimiento?',
      options: [
        '"I hear you. What were you expecting to pay? [Let her answer] Right — and this is a year of it, not one session." Then hold.',
        'Go straight down two rungs to {currency}140 and put the voucher on it, so the number stops being the thing standing between you and the sale',
        'Tell her she is wrong and that this is genuinely one of the cheapest professional treatments that anybody is selling anywhere on this coast',
        'Let her go and find an easier customer, because anybody who says that at the first price was never really going to buy',
      ],
      optionsEs: [
        '"Te escucho. ¿Cuánto esperabas pagar? [Deja que conteste] Ya — y esto es un año entero, no una sesión." Y ahí te quedas.',
        'Bajar dos escalones de golpe hasta {currency}140 y ponerle el cupón, para que la cifra deje de ser lo que se interpone entre tú y la venta',
        'Decirle que se equivoca y que este es de verdad de los tratamientos profesionales más baratos que vende nadie en toda esta costa',
        'Dejarla marchar y buscar una clienta más fácil, porque quien dice eso al primer precio no iba a comprar de todas formas',
      ],
      correctIndex: 0,
      explanation: 'The first time you hear it, you do not move the number — you make her ask again. And notice where you are standing: at {currency}210 there is a lot of ladder left, so holding costs you nothing. That answer changes further down. By {currency}175 or {currency}140 she has already refused the big numbers, and sitting still is not confidence any more — down there you move, with a reason attached: gift off, voucher on, scrub back as credit. Never a drop with no story behind it.',
      explanationEs: 'La primera vez que lo oyes no mueves el número — haces que te lo pida otra vez. Y fíjate dónde estás: en {currency}210 queda mucha escalera, así que aguantar no te cuesta nada. Más abajo la respuesta cambia. En {currency}175 o {currency}140 ya ha rechazado los números grandes, y quedarte quieto ahí ya no es seguridad — ahí te mueves, con un motivo pegado: fuera el regalo, dentro el cupón, el exfoliante de vuelta como crédito. Nunca una bajada sin una historia detrás.',
    },
    {
...
const workflowQuiz: GeneralQuiz = {
  ...
  description: 'Master door rules, rotation, demo steps, and the handover to the specialist once the syringe is sold.',
  descriptionEs: 'Domina las reglas de puerta, la rotación, los pasos de la demo y el traspaso al especialista una vez vendida la jeringa.',
  icon: 'Workflow',
  category: 'Workflow',
  ...
  questions: [
    ...
    {
      question: 'She has paid for the syringe and she is still in the chair. Now what?',
      questionEs: 'Ha pagado la jeringa y sigue sentada en la silla. ¿Ahora qué?',
      options: [
        'Suggest one complementary product — a scrub, a butter, a nail kit — to grow the basket a little',
        'Give the usage instructions, then hand her over to the specialist',
        'Walk her over to the specialist yourself so she does not have to go looking around for him',
        'Thank her, wrap it, and get straight back out on the door before somebody else takes your turn',
      ],
      optionsEs: [
        'Sugerirle un producto complementario — exfoliante, manteca, kit de uñas — para subir un poco la cesta',
        'Darle las instrucciones de uso y traspasarla al especialista',
        'Acompañarla tú misma hasta el especialista para que no tenga que ir buscándolo por ahí',
        'Darle las gracias, envolverlo y volver a la puerta antes de que otro te quite el turno',
      ],
      correctIndex: 1,
      explanation: 'Your job ends at the syringe, not at the money — and it does not turn into a second pitch. Instructions, then the handover: build the specialist up, give her a small gift, ask which she would rather have lifted, and go and fetch him. She never stands up. Anybody on her feet with a bag in her hand is halfway to the door.',
      explanationEs: 'Tu trabajo se acaba en la jeringa, no en el dinero — y no se convierte en un segundo discurso. Instrucciones y traspaso: móntale al especialista, hazle un regalito, pregúntale qué se levantaría, y ve tú a buscarlo. Ella no se levanta. Cualquiera de pie con la bolsa en la mano ya va camino de la puerta.',
    },
    {
